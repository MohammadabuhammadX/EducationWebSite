...
namespace DAL
{
    public class MetaDAO : PostContext
    {
        ...
        public List<MetaDTO> GetMetaData()
        {
            ...
            List<Meta> list = db.Metas.Where(x => x.isDeleted == false).OrderBy(x => x.AddDate).ToList();      // now we have a metalist , but we have to return metadata list, so we have to convert              
            foreach (var item in list)
            {
                MetaDTO dto = new MetaDTO();                    // now we will assign all necessary values to DTO
                dto.MetaID = item.ID;
                dto.Name = item.Name;
