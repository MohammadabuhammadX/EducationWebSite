...
namespace UI.Areas.Admin.Controllers
{
    ...
    public class PostController : BaseController             // Post Table is main table , So i'll choose Make this to be the last Page
    {
        ...
        public ActionResult AddPost()           // In this action we will send PostDTO to the View.
        {
            PostDTO model = new PostDTO();
            model.Categories = CategoryBLL.GetCategoriesForDropdown();
            return View(model);                             // Now there's a difference form the other actions , We've got to set category ,List  and model to show it in the page ,So let's get categories with select item list.
        ...
        public ActionResult AddPost(PostDTO model)
        {
            ...
            else if (ModelState.IsValid)
            {
                ...
                foreach (var postfile in model.PostImage)
                {
                    Bitmap image = new Bitmap(postfile.InputStream);
                    Bitmap resizeimage = new Bitmap(image, 750, 422);
                    string filename = "";
                    string uniqueNumber = Guid.NewGuid().ToString();
                    filename = uniqueNumber + postfile.FileName;
